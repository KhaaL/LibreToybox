...
- **Long-press a placed stamp to remove it** — `onPointerDown` hit-tests the tap point against `stamps` (topmost/last-in-array first, matching what's visually on top) via `findStampAt()`; a hit starts `removing` instead of a new placement. Holding past `LONG_PRESS_MS` (550 ms) removes it; releasing early or drifting past `TAP_MOVE_PX` cancels with no effect. Since stamps are canvas-drawn text, not DOM nodes, there's no element to attach Sudoku's `.conflict`/`flashShake` or `.cleared`/`suction` CSS animations to — `tickRemoveHold()` and `animateRemoval()` hand-roll the same two beats as canvas frames instead: a reddish pulsing ring + shake that both grow with hold progress (the "about to be removed" warning), then a shrink-and-fade-out (`drawStamp`'s new `scale` param) plus the existing `audioSwoosh()` once the hold completes.
