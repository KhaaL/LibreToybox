...
- **Win-screen "odd ones" recap** (`oddsFound`, `renderOddsRecap()`): every correct pick appends `this.tiles[this.oddIndex]` to `oddsFound` (reset in `newGame()` alongside `solved`); `celebrate()` renders it into `#odds-recap` as a small grid using that difficulty's own `CONFIG.sizes[size].cols` — the same column count `renderBoard()` uses — so the recap echoes the board's own tile-grid shape rather than inventing new layout. Styled muted/desaturated (`.recap-tile`: translucent white, lower opacity, `grayscale` filter) so it stays visually secondary to the bright, saturated `.emoji-row` celebration above it.
